    ...
    public UserBal(UserDal userDal){
        ...
        public User registeruUser(User user){
            if(userDal.existsByEmail(user.getEmail())){
                throw new DuplicateResourceException("email already in use:"+user.getUsername());
            }
            if(userDal.existsByUsername(user.getUsername())){
                throw new DuplicateResourceException("isername already in use: "+user.getUsername());
            }
